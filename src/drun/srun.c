...
// now not static to be able to be used in drun
srun_job_t *job = NULL;
// now global to be able to be used in drun
...
int srun(int ac, char **av)
{
	...
	if (opt_list) {
		info("with opt_list; HETJOB launch");
		...
		info("with NO opt_list; regular launch");
		create_srun_job((void **) &job, &got_alloc);
	}
	// the create srun job calls above are the ones that interact with the
	// scheduler to get the concrete nodes and processes per node, etc.
	// once this is done, the "job" is populated with the step_ctx
	...
	// this call results in a call to:
	// launch_g_step_launch()
	// - from src/srun/launch.c
	// which may call internally:
	// slurm_step_launch if it is the first time, or
	// slurm_step_launch_add if it is not the first time
	// - from src/api/step_launch.c
	// this means that we may only need to call this again to get
	// any extra processes created, but this is probably never
	// tested so may be buggy.  In addition to this, there is
	// an IO setup created, and it seems that the top call
	// does not check if its the first time, which may result
	// in the IO server thread being overwritten
	// the io is set in the context
	// - from src/api/step_io.c
	// TODO these files may need to be copied and modified
	info("########### replacing launch logic ###############");
	info("_launch_app in srun.c");
	_launch_app(job, srun_job_list, got_alloc);
	info("done _launch_app in srun.c");
	info("########### done replacing launch logic ###############");
...
static void _launch_app(srun_job_t *job, List srun_job_list, bool got_alloc)
{
	...
	info("nodelist: %s", job->step_ctx->step_resp->step_layout->node_list);
	...
	if (srun_job_list) {
		...
		info("single launch: nodelist: %s", job->step_ctx->step_resp->step_layout->node_list);
		opts = xmalloc(sizeof(_launch_app_data_t));
		opts->got_alloc   = got_alloc;
		opts->job         = job;
		opts->opt_local   = &opt;
