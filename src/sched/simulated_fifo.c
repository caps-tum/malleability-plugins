...
static int	_schedule(bool full_queue, time_t now);
...
static int _schedule(bool full_queue, time_t now)
{
	...
	// we changed this to provide the time for now arbitrarily
	// from the simulator time loop
	//time_t now, last_job_sched_start, sched_start;
	time_t last_job_sched_start, sched_start;
	...
	if (sched_update != slurm_conf.last_update) {
		debug("scheduler is updating its configuration");
		char *tmp_ptr;
		...
		if (bf_hetjob_immediate && !bf_hetjob_prio) {
			bf_hetjob_prio |= HETJOB_PRIO_MIN;
			debug3("bf_hetjob_immediate automatically sets bf_hetjob_prio=min");
		}
	...
	if ((defer_rpc_cnt > 0) &&
		...
		error("schedule() returning, too many RPCs");
	...
	if (!fed_mgr_sibs_synced()) {
		error("schedule() returning, federation siblings not synced yet");
		goto out;
	}

	// TODO we never lock anything in simulator mode
	//lock_slurmctld(job_write_lock);

	// time now was set here with time()
	sched_start = now;
	last_job_sched_start = now;
	START_TIMER;
	if (!avail_front_end(NULL)) {
		error("there should be no frontend");
		...
		// see todo above
		//unlock_slurmctld(job_write_lock);
		error("schedule() returning, no front end nodes are available");
		goto out;
	}
	...
	if (!reduce_completing_frag && job_is_completing(NULL)) {
		//see todo above
		//unlock_slurmctld(job_write_lock);
		error("schedule() returning, some job is still completing");
		goto out;
	}
	...
	debug("Running job scheduler %s.", full_queue ? "for full queue":"for default depth");
	/*
...
next_task:
		// this check is disabled in simulator mode
		//if ((time(NULL) - sched_start) >= sched_timeout) {
		//	error("loop taking too long, breaking out");
		//	break;
		//}
		if (sched_max_job_start && (job_cnt >= sched_max_job_start)) {
			error("sched_max_job_start reached, breaking out");
			break;
		}
...
skip_start:
		...
		if (error_code == ESLURM_NODES_BUSY) {
			...
			info("Allocate %pJ NodeList=%s #CPUs=%u Partition=%s",
				   job_ptr, job_ptr->nodes,
				   job_ptr->total_cpus,
				   job_ptr->part_ptr->name);
			debug2("time limit set to: %d", job_ptr->time_limit);
			debug2("start time set to: %d", job_ptr->start_time);
			job_ptr->start_time = job_ptr->time_last_active = now;
			debug("reset start time to: %d", job_ptr->start_time);
			if (job_ptr->batch_flag == 0){
				debug2("batch flag set; calling srun_allocate()");
				srun_allocate(job_ptr);}
			else if (!IS_JOB_CONFIGURING(job_ptr)){
				debug2("job is configuring; calling launch_job()");
				launch_job(job_ptr);}
...
out:
    //info("returning %d at now %d", job_cnt, now);
	return job_cnt;
}
...
{
	// TODO double check if there is any necessary state set here
	// skipping this in simulator mode
	batch_job_launch_msg_t *launch_msg_ptr;
...
extern void reboot_job_nodes(job_record_t *job_ptr)
{
	// in the simulator, this is skipped
	debug("skipping this for now");
	return;
...
// TODO may need versions of these for the simulator
...
// this is used to ensure a pass takes place regardless
// of the time interval; emulates what occurs on a real
// job submission event, that ensures a job start if there
// are sufficient resources
void fifo_pass_immediate(time_t now){
    _schedule(0, now);
    debug("### fifo_pass_inmediate performed at time %d ###", now);
}
...
void fifo_pass(time_t now) {
	...
	debug3("sched_min_interval: %d", sched_min_interval);
	debug3("sched_full_queue: %d", sched_full_queue);
	debug3("full_queue: %d", full_queue);
	debug3("sched_requests: %d", sched_requests);
	debug3("last_scheduled: %d", last_scheduled);
	debug3("delta_t: %d", delta_t);

	// TODO check if we need to track sched_requests
	// this is likely an optimization, where the scheduler pass
	// is skipped unless there is at least one request queued
	// during the regular thread operation (i.e. not this
	// simulator hack).
	...
	if (delta_t > sched_min_interval ) {
		...
		sched_requests = 0;
		 ...
		 _schedule(full_queue, now);

		last_scheduled = now;

		debug("### fifo_pass performed at time %d ###", now);
	} else {
		debug2("### no fifo_pass performed at time %d ###", now);
	}
}
