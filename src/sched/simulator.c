/*****************************************************************************\
 *  This is a modified version of the controller.c file from Slurm,
 *  that is stripped down of some functionality not needed for simulations.
 *
 *  simulator.c - main control machine daemon for slurm
 *****************************************************************************
...
char const plugin_type[] = "sched"; // deepsea added as a workaround
...
int main(int argc, char **argv)
{
	...
	//}
	// we do not run as daemon when in simulator mode
	slurmctld_config.daemonize = 0;
	// deepsea changes end
	...
	//}
	// deepsea changes begin
	// node management does not need to happen in simulator mode
	//agent_init();
	// deepsea changes end
	...
	while (1) {
		...
		if (!slurmctld_primary) {
			error("not primary; should never happen in simulator mode");
		...
		// TODO check power management logic; seems to run on a thread
		// that grabs node_write_lock and may trigger comms
		//config_power_mgr();
		 ...
		 */
		// deepsea changes begin
		// removing RPC handler in simulator mode
		//server_thread_incr();
		...
		//info("grabbing locks forever"); fflush(0);
		//lock_slurmctld(all_locks);

		info("before sched_g_init");
		if (sched_g_init() != SLURM_SUCCESS)
			fatal("failed to initialize scheduling plugin");
		info("after sched_g_init");
		// deepsea changes end
...
static void *_slurmctld_background(void *no_data)
{
	...
	while (1) {
		info("doing something in the periodic loop"); fflush(0); sleep(1);
		...
		//	info("locking node_write_lock");
		//	lock_slurmctld(node_write_lock);
		//	info("checking set_node_maint_mode");
		//	now = time(NULL);
		//	info("2"); fflush(0);
		//	last_resv_time = now;
		//	info("3"); fflush(0);
		...
		//	job_time_limit();
		//	job_resv_check();
		//	info("unlocking job_write_lock");
		//	unlock_slurmctld(job_write_lock);

		//	info("locking node_write_lock");
		//	lock_slurmctld(node_write_lock);
		//	info("checking check_reboot_nodes");
		//	check_reboot_nodes();
		//	info("unlocking node_write_lock");
		//	unlock_slurmctld(node_write_lock);
		//}
		...
		//	     HEALTH_CHECK_CYCLE) {
		//		info("doing nothing to check node health");
		//		/* Call run_health_check() on each cycle */
		//	} else {
		//		info("setting time to check node health");
		//		now = time(NULL);
		...
		//	info("updating node acc gather data");
		...
		//	info("updating sensors component data");
		...
		//	info("locking job_read_lock");
		//	lock_slurmctld(job_read_lock);
		//	now = time(NULL);
		//	last_ping_srun_time = now;
		//	debug2("Performing srun ping");
		//	srun_ping();
		//	info("unlocking job_read_lock");
		//	unlock_slurmctld(job_read_lock);
		//}

		// we should not interact with nodes in simulator mode,
		// therefore also no reboot messaging
		//if (want_nodes_reboot) {
		//	info("locking node_write_lock");
		//	lock_slurmctld(node_write_lock);
		//	info("sending reboot message");
		//	_queue_reboot_msg();
		//	info("unlocking node_write_lock");
		//	unlock_slurmctld(node_write_lock);
		//}
		...
		//	slurm_mutex_unlock(&sched_cnt_mutex);
		//}
		// we don't schedule in the main timer
		// we schedule in the simulator with the FIFO pass call
		//if (call_schedule) {
		//	info("locking job_write_lock2");
		//	lock_slurmctld(job_write_lock2);
		//	now = time(NULL);
		//	last_sched_time = now;
		//	bb_g_load_state(false);	/* May alter job nice/prio */
		//	info("unlocking job_write_lock2");
		...
		//	info("locking job_node_read_lock");
		//	lock_slurmctld(job_node_read_lock);
		//	now = time(NULL);
		//	last_trigger = now;
		//	trigger_process();
		//	info("unlocking job_node_read_lock");
		...
		//	info("Performing full system state save");
		//	save_all_state();
		//}
		...
		//	(void) _shutdown_backup_controller();
		//}
		//info("unlocking config_read_lock");
		//unlock_slurmctld(config_read_lock);
...
static void _run_primary_prog(bool primary_on)
{
	...
	if ((prog_name == NULL) || (prog_name[0] == '\0')){
		info("prog_name was not set");
		return;
	} else {
		info("running as: %s", prog_name);
	}
	// deepsea changes end
