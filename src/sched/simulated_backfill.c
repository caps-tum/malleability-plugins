...
static void _attempt_backfill(time_t);
...
static int  _start_job(job_record_t *job_ptr, bitstr_t *avail_bitmap, time_t);
...
static int  _try_sched(job_record_t *job_ptr, bitstr_t **avail_bitmap,
		       ...
		       uint32_t req_nodes, bitstr_t *exc_core_bitmap, time_t);
...
static int  _try_sched(job_record_t *job_ptr, bitstr_t **avail_bitmap,
		       uint32_t min_nodes, uint32_t max_nodes,
		       uint32_t req_nodes, bitstr_t *exc_core_bitmap, time_t now)
{
...
static void _attempt_backfill(time_t now) {
	DEF_TIMERS;
	List job_queue;
	...
	time_t sched_start, later_start, start_res, resv_end, window_end;
	...
	sched_start = orig_sched_start = now;
	...
	if (job_test_count == 0) {
        verbose("no jobs to backfill");
		FREE_NULL_LIST(job_queue);
		return;
	} else
		verbose("%u jobs to backfill", job_test_count);
    ...
    //verbose("1: avail_node_bitmap: %d", bit_set_count(avail_node_bitmap));
	/* Make "resuming" nodes available to be scheduled in backfill */
	bit_or(node_space[0].avail_bitmap, rs_node_bitmap);
    //verbose("1: node_space[0].avail_bitmap: %d", bit_set_count(node_space[0].avail_bitmap));
    ...
    // TODO find a better way to control verbosity here;
    // since this debug flag is separate from the -Dvvvv format
	...
	while (1) {
		...
		if (job_ptr) {
            debug("doing basic preparations for job %d", job_ptr->job_id);
		...
		if (!job_queue_rec) {
			debug("reached end of job queue");
			break;
		}

		if (slurmctld_diag_stats.bf_last_depth_try >=
		    max_backfill_job_cnt) {
			debug("bf_max_job_test: limit of %d reached",
				 max_backfill_job_cnt);
			break;
		}
		...
		if (is_job_array_head &&
			...
			debug("%pJ array scheduled during bf yield, try master",
				 job_ptr);
		...
		if (!_job_runnable_now(job_ptr)){
            verbose("job %d is not runnable, continue with next", job_ptr->job_id);
			continue;
        }
		if (!part_ptr){
            verbose("partition not set for job %d, continue with next", job_ptr->job_id);
			continue;
        }
		...
		if (het_job_time > (now + backfill_window)){
            debug("het_job_time > (now + backfill_window");
			continue;
        }

		if (job_ptr->qos_id) {
            error("has qos, should not in simulator");
		...
		//assoc_mgr_lock(&qos_read_lock);
		if (job_ptr->qos_ptr) {
            error("has qos 2, should not in simulator");
		...
		if (part_policy_valid_qos(job_ptr->part_ptr, job_ptr->qos_ptr,
            ...
            debug("invalid qos for job %d, continue with next", job_ptr->job_id);
			//assoc_mgr_unlock(&qos_read_lock);
			continue;
		}
		//assoc_mgr_unlock(&qos_read_lock);
		...
		if (!assoc_limit_stop &&
            ...
            debug("not runnable pre select, continue with next");
			continue;
		}
		...
		if (!job_ptr->part_ptr->bf_data &&
		    (bf_job_part_count_reserve ||
		     max_backfill_job_per_user_part ||
		     max_backfill_job_per_part)) {
            debug("initializing job_ptr->part_ptr->bf_data");
		...
		if ((job_no_reserve == 0) && bf_job_part_count_reserve) {
            debug("(job_no_reserve == 0) && bf_job_part_count_reserve");
			if (_check_bf_usage(
		...
		if (job_ptr->preempt_in_progress){
            error("preempt_in_progress was set");
...
next_task:
		 ...
		 */
		if (job_ptr->preempt_in_progress) {
            error("job_ptr->preempt_in_progress");
		...
		verbose("test for %pJ Prio=%u Partition=%s",
		...
		if (!bf_licenses &&
			...
			debug("%pJ not runable now due to licenses",
				 job_ptr);
			continue;
		}

		if (!job_independent(job_ptr)) {
			debug("%pJ not runable now",
				 job_ptr);
			continue;
		}
		...
		if (error_code == ESLURM_ACCOUNTING_POLICY) {
			debug("%pJ acct policy node limit",
				 job_ptr);
			continue;
		} else if (error_code ==
			   ESLURM_REQUESTED_PART_CONFIG_UNAVAILABLE) {
			debug("%pJ node count too high",
				 job_ptr);
			continue;
		} else if (error_code != SLURM_SUCCESS) {
			debug("error setting nodes for %pJ: %s",
		...
		if (assoc_limit_stop) {
			if (qos_blocked_until > later_start) {
				...
				debug("QOS blocked_until move start_res to %ld",
					 later_start);
			}
			if (qos_part_blocked_until > later_start) {
				later_start = qos_part_blocked_until;
				debug("Part QOS blocked_until move start_res to %ld",
					 later_start);
			}
		}
 ...
 TRY_LATER:
        ...
        // do not check shutdown option in simulator
        if(difftime(now, orig_sched_start) >=
		     bf_max_time) {
            debug("breaking on the difftime check");
            //info("bf_max_time: %d", bf_max_time);
            //info("now: %d", now);
            //info("orig_sched_start: %d", orig_sched_start);
		...
		many_rpcs = false;
        // we don't track if there are many rpcs in the simulator, since
        // there will never be RPCS
		//slurm_mutex_lock(&slurmctld_config.thread_count_lock);
		//if ((max_rpc_cnt > 0) &&
		//    (slurmctld_config.server_thread_count >= max_rpc_cnt))
		//	many_rpcs = true;
        ...
        //verbose("1: job_test_resv, avail_bitmap out: %d", bit_set_count(avail_bitmap));
		if (j != SLURM_SUCCESS) {
			verbose("%pJ reservation defer",
				 job_ptr);
			_set_job_time_limit(job_ptr, orig_time_limit);
			continue;
		}
        ...
        //verbose("3: part_ptr->node_bitmap: %d", bit_set_count(part_ptr->node_bitmap));
        //verbose("3: avail_bitmap: %d", bit_set_count(avail_bitmap));
		bit_and(avail_bitmap, up_node_bitmap);
        //verbose("3: up_node_bitmap: %d", bit_set_count(up_node_bitmap));
        //verbose("3: avail_bitmap: %d", bit_set_count(avail_bitmap));
		bit_and_not(avail_bitmap, bf_ignore_node_bitmap);
        //verbose("3: bf_ignore_node_bitmap: %d", bit_set_count(bf_ignore_node_bitmap));
        //verbose("3: avail_bitmap: %d", bit_set_count(avail_bitmap));
		filter_by_node_owner(job_ptr, avail_bitmap);
        //verbose("4:filter_by_node_owner avail_bitmap: %d", bit_set_count(avail_bitmap));
		filter_by_node_mcs(job_ptr, mcs_select, avail_bitmap);
        //verbose("4:filter_by_node_mcs avail_bitmap: %d", bit_set_count(avail_bitmap));
		tmp_bitmap = bit_copy(avail_bitmap);
        //verbose("4:tmp bitmap: %d", bit_set_count(tmp_bitmap));
		for (j = 0; ; ) {
		...
		if (resv_end && (++resv_end < window_end) &&
            ...
            debug("later_start = resv_end");
			later_start = resv_end;
		}

		if (job_ptr->details->exc_node_bitmap) {
            debug("job_ptr->details->exc_node_bitmap");
		...
		if (licenses_unavail ||
            ...
            if(licenses_unavail) error("licenses_unavail");
            if(bit_set_count(avail_bitmap) < min_nodes)
                verbose("---- insuficient nodes for job %d (avail: %d, min: %d), continue with next",
                        job_ptr->job_id, bit_set_count(avail_bitmap), min_nodes);
		...
		verbose("entering _try_sched for %pJ.",
		       job_ptr);
		...
		if (active_bitmap) {
			...
			j = _try_sched(job_ptr, &active_bitmap, min_nodes,
				       max_nodes, req_nodes, exc_core_bitmap, now);
			if (j == SLURM_SUCCESS) {
				FREE_NULL_BITMAP(avail_bitmap);
		...
		if (test_fini == 0) {
			...
			verbose("entering _try_sched for %pJ. Need to use features which can be made available after node reboot",
			       job_ptr);
			/* Determine impact of any advance reservations */
			resv_end = 0;
		...
		if ((job_ptr->start_time <= now) &&
			...
			verbose("future start: %pJ. State=%s. Reason=%s. Priority=%u.",
				     job_ptr,
			...
			if (fed_mgr_job_lock(job_ptr)) {
				debug("%pJ can't get fed job lock from origin cluster to backfill job",
					 job_ptr);
				rc = ESLURM_FED_JOB_LOCK;
				goto skip_start;
			}

			rc = _start_job(job_ptr, resv_bitmap, now);
...
skip_start:
        debug("skip_start");
			...
			if ((rc == ESLURM_RESERVATION_BUSY) ||
				...
				verbose("planned start of %pJ failed: %s",
				...
				if (max_backfill_jobs_start &&
					...
					verbose("bf_max_job_start limit of %d reached",
						 max_backfill_jobs_start);
					break;
				}
				if (is_job_array_head &&
		...
		if ((job_ptr->start_time > now) && (job_no_reserve != 0)) {
			if ((orig_start_time != 0) &&
				...
				verbose("%pJ StartTime set but no backfill reservation created.",
					 job_ptr);
			}
			_set_job_time_limit(job_ptr, orig_time_limit);
			continue;
		...
		if (later_start && (job_ptr->start_time > later_start)) {
			...
			verbose("Try later %pJ later_start %ld",
			         job_ptr, later_start);
			job_ptr->start_time = 0;
			goto TRY_LATER;
		}
		...
		if (job_ptr->start_time > (sched_start + backfill_window)) {
			...
			if ((orig_start_time != 0) &&
				...
				verbose("%pJ StartTime set to time after current backfill window. No reservation created",
					 job_ptr);
			}
			_set_job_time_limit(job_ptr, orig_time_limit);
			continue;
		...
		if ((job_ptr->start_time > now) &&
			...
			job_ptr->start_time = 0;
			verbose("%pJ overlaps with existing reservation start_time=%u end_reserve=%u boot_time=%u later_start %ld",
		...
		if (!assoc_limit_stop) {
			...
			if (!acct_policy_job_runnable_post_select(job_ptr,
							  tres_req_cnt, true)) {
				assoc_mgr_unlock(&locks);
				verbose("adding reservation for %pJ blocked by acct_policy_job_runnable_post_select",
					 job_ptr);
				_set_job_time_limit(job_ptr, orig_time_limit);
				continue;
			}
		...
		if ((!bf_one_resv_per_job || !orig_start_time) &&
			...
			if (node_space_recs >= bf_node_space_size) {
				debug("table size limit of %u reached",
					 bf_node_space_size);
				if ((max_backfill_job_per_part != 0) &&
				    (max_backfill_job_per_part >=
	...
	//if (slurm_conf.debug_flags & DEBUG_FLAG_BACKFILL) {
		//END_TIMER;
		debug("completed testing %u(%d) jobs, %s",
		     slurmctld_diag_stats.bf_last_depth,
		     job_test_count, TIME_STR);
	//}
	...
	//if (slurmctld_config.server_thread_count >= 150) {
		debug("%d pending RPCs at cycle end, consider "
		     "configuring max_rpc_cnt",
		     slurmctld_config.server_thread_count);
	//}
	//slurm_mutex_unlock(&slurmctld_config.thread_count_lock);

    debug("ending backfiller pass");
}

/* Try to start the job on any non-reserved nodes */
static int _start_job(job_record_t *job_ptr, bitstr_t *resv_bitmap, time_t now)
{
	int rc;
	...
	if (rc == SLURM_SUCCESS) {
		...
		info("Jobs backfilled since boot: %u",
			 slurmctld_diag_stats.backfilled_jobs);
...
static int _het_job_start_now(het_job_map_t *map, node_space_map_t *node_space)
{
	...
	while ((rec = list_next(iter))) {
		...
		avail_bitmap = NULL;
		bit_not(resv_bitmap);
        // TODO hetjobs should never be encountered in simulation
        error("encountered a hetjob");
        exit(-1);
...
void backfill_init() {
    ...
    info("backfill_interval after init: %d", backfill_interval);
}
...
// this is a copy of the agent code, modified to perform a single backfill pass,
// instead of the timed and event based behaviour for the deepsea simulator
void backfill_pass(time_t now) {
	static time_t last_scheduled = 0;

	time_t delta_t = now - last_scheduled;
	last_scheduled = now;

    // TODO do we really need to flush here?
	list_flush(het_job_list);

    if (delta_t >= backfill_interval){
        _attempt_backfill(now);
        debug("### backfill_pass performed at time %d ###", now);
    } else {
        debug2("### no backfill_pass performed at time %d ###", now);
    }
}
