...
// we may decide to remove code that is not used in our
// dynamic resources environment, since a lot is
// replaced directly or via libdynpm

//#include "zlib.h"
#include <bzlib.h>
#include "src/common/slurm_protocol_pack.h"
#include "drun.h"
// these are created in drun.c
extern drun_tbon_data_t slurmd_tbon_data;
extern drun_tbon_data_t slurmstepd_tbon_data;
extern drun_step_data_t step_data;
...
static void _wait_tbon_cond_ready(drun_tbon_data_t *tbon_data){
	pthread_mutex_lock(&tbon_data->mutex);
	if(!tbon_data->ready){
		info("slurmd waiting for tbon");
		...
		pthread_cond_timedwait(&(tbon_data->cond), &(tbon_data->mutex), &tbon_wait_timeout);
	} else {
		info("slurmd tbon was ready before we waited on cond");
	}
	pthread_mutex_unlock(&tbon_data->mutex);
}

static void _wait_tbon_cond_ready_step(drun_step_data_t *step_data){
	...
	if(!step_data->ready){
		...
		pthread_cond_timedwait(&(step_data->cond), &(step_data->mutex), &tbon_wait_timeout);
	} else {
		info("slurmd tbon was ready before we waited on cond");
	}
			     ...
			     const slurm_step_launch_callbacks_t *callbacks)
{
	...
	// TODO need to keep these callbacks visible at the
	// drun handlers, so that we can call them based on our
	// own libdynpm network events
	// TODO look at:
	// slurm_step_launch_callbacks_t in slurm/slurm.h
	...
	// deep-sea:
	// TODO check if we need to disable wrapping of the host mpi/<pmi type>
	// here in drun (usually <default>); we disable it in dstepd
	...
	if (params->het_job_ntasks != NO_VAL)
		preserve_env = true;
	// TODO verify that we are safe in omiting the launch_state
	...
	// TODO add validation here; only the mpi/deepsea id is valid for drun
			 ...
			 uint32_t timeout, char *nodelist)
{
	...
	while ((name = hostlist_shift(hl))) {
		_print_launch_msg(launch_msg, name, host_count++);
		free(name);
	}
	hostlist_destroy(hl);
	info("total hosts: %d", host_count);
	...
	if(pack_msg(&msg, buffer)){
		error("failed to pack launch_msg");
		return -1;
	} else { info("packed the launch message; size: %d", buffer->size); }

	//for(int i = 0; i < 10; i++){ info("%2x", buffer->head[i]); }
	//printf("\n");

	//info("uncompressed: %s", buffer.head);
	char *compressed_buffer = malloc(4096);
	unsigned length = 4096;
	//if(compress2(compressed_buffer, &length, buffer.head, buffer.size, 9)){
	//if(compress(compressed_buffer, &length, buffer.head, buffer.size)){
	//	error("could not compress source buffer");
	//	return SLURM_ERROR;
	//}
	if(BZ2_bzBuffToBuffCompress(compressed_buffer, &length, buffer->head, buffer->size, 9, 0, 0)){
		error("could not compress source buffer");
		return SLURM_ERROR;
	}
	info("compressed to size: %d", length);

	//for(int i = 0; i < 10; i++){ info("%2x", compressed_buffer[i]); }
	//info("%2x", compressed_buffer[length-1]);

	//char *decompressed_buffer = malloc(buffer->size);
	//unsigned dlen = buffer->size;
	//if(BZ2_bzBuffToBuffDecompress(decompressed_buffer, &dlen, compressed_buffer, length, 0, 0)){
	//	error("could not uncompress");
	//	return SLURM_ERROR;
	//}
	...
	if(!slurmd_tbon_data.started){
		info("tbon was not started yet; beginning the creation process");
		slurmd_tbon_data.started = 1;
		//slurmd_tbon_data.hosts = strdup(job->step_ctx->step_resp->step_layout->node_list);
		slurmd_tbon_data.node_count = ctx->step_resp->step_layout->node_cnt;
		slurmd_tbon_data.nodes = strdup(nodelist);
		xassert(slurmd_tbon_data.master_host == NULL);
		// first, we mark our master node
		hostlist_t *hl = hostlist_create(slurmd_tbon_data.nodes);
		char *master_host = hostlist_shift(hl);
		slurmd_tbon_data.master_host = strdup(master_host);
		free(master_host);
		hostlist_destroy(hl);
		info("master_host: %s", slurmd_tbon_data.master_host);
		...
		char port[32];
		sprintf(port, "%d", slurmd_config_entry->port);
		slurmd_tbon_data.port = strdup(port);
		if(dynpm_connect_to_server(slurmd_tbon_data.master_host, slurmd_tbon_data.port,
					_slurmd_client_handler, LAUNCHER, NULL,
					&(slurmd_tbon_data.master_host_server_index))){
			error("could not connect to master host");
			return SLURM_ERROR;
		}

		// tbon create command sent here
		dynpm_header_t *tbon_header = malloc(sizeof(dynpm_header_t));
		tbon_header->source_type =  (dynpm_source_type)LAUNCHER;
		tbon_header->message_type = (dynpm_message_type)CREATE_TBON;

		dynpm_create_tbon_msg_t *tbon_create = malloc(sizeof(dynpm_create_tbon_msg_t));
		tbon_create->id = ctx->step_req->step_id.step_id;
		tbon_create->degree = 2; // going for minimum for testing
		xassert(tbon_create->degree >= 2); // no point in degree less than 2
		tbon_create->total = host_count;
		tbon_create->nodes = strdup(slurmd_tbon_data.nodes);

		if(dynpm_send_message_to_server(&tbon_header, (void**)&tbon_create,
					slurmd_tbon_data.master_host_server_index)){
			error("sending tbon create master host %s", slurmd_tbon_data.master_host);
		} else {
			info("sent tbon create master host %s", slurmd_tbon_data.master_host);
		}
	} else {
		error("tbon was started before initial launch");
		return SLURM_ERROR;
	}

	pthread_mutex_unlock(&slurmd_tbon_data.mutex);

	_wait_tbon_cond_ready(&slurmd_tbon_data);
	info("done waiting for tbon");

	info("===============================  DONE SLURMD TBON  ==============================");

	info("===============================  BEGIN DYNAMIC LAUNCH  ==============================");
	pthread_mutex_lock(&slurmd_tbon_data.mutex);
	info("slurmd tbon is ready; broadcasting dynamic launch...");

	dynpm_header_t *dynamic_launch_header = malloc(sizeof(dynpm_header_t));
	dynamic_launch_header->source_type =  (dynpm_source_type)LAUNCHER;
	dynamic_launch_header->message_type = (dynpm_message_type)DYNAMIC_PM_LAUNCH;
	dynpm_dynamic_pm_launch_msg_t *dynamic_launch = malloc(sizeof(dynpm_dynamic_pm_launch_msg_t));
	dynamic_launch->id = 0;
	dynamic_launch->type = 0;
	dynamic_launch->payload_bytes = length;
	dynamic_launch->payload = compressed_buffer;

	// single message to the node master; it should broadcast over the tbon
	if(dynpm_send_message_to_server(&dynamic_launch_header, (void**)&dynamic_launch,
				slurmd_tbon_data.master_host_server_index)){
		error("sending dynamic launch to master host %s index %d",
				slurmd_tbon_data.master_host, slurmd_tbon_data.master_host_server_index);
	} else {
		info("sent dynamic launch to master host %s index %d",
				slurmd_tbon_data.master_host, slurmd_tbon_data.master_host_server_index);
	}

	pthread_mutex_unlock(&slurmd_tbon_data.mutex);

	_wait_tbon_cond_ready_step(&step_data);

	step_data.job_index = ctx->job_id;
	step_data.application_index = ctx->step_resp->job_step_id;
	step_data.malleability_mode = 0;
	step_data.node_count = ctx->step_resp->step_layout->node_cnt;
	step_data.nodes = strdup(nodelist);
	step_data.process_count	= ctx->step_resp->step_layout->task_cnt;

	info("===============================  END DYNAMIC LAUNCH  ==============================");

	if(!slurmstepd_tbon_data.started){
		info("stepd tbon was not started yet; beginning the creation process");
		slurmstepd_tbon_data.started = 1;
		//slurmstepd_tbon_data.hosts = strdup(job->step_ctx->step_resp->step_layout->node_list);
		slurmstepd_tbon_data.node_count = ctx->step_resp->step_layout->node_cnt;
		slurmstepd_tbon_data.nodes = strdup(nodelist);
		xassert(slurmstepd_tbon_data.master_host == NULL);
		// first, we mark our master node
		hostlist_t *hl = hostlist_create(slurmstepd_tbon_data.nodes);
		char *master_host = hostlist_shift(hl);
		slurmstepd_tbon_data.master_host = strdup(master_host);
		free(master_host);
		hostlist_destroy(hl);
		info("stepd master_host: %s", slurmstepd_tbon_data.master_host);

		// first connect to the slurmd at the master host
		dynpm_process_conf_entry_t *slurmstepd_config_entry;
		if(dynpm_conf_get_process_config(&slurmstepd_config_entry, "slurmstepd" )){
			error("could not find slurmstepd's configuration");
			return SLURM_ERROR;
		}

		char port[32];
		sprintf(port, "%d", slurmstepd_config_entry->port);
		slurmstepd_tbon_data.port = strdup(port);
		if(dynpm_connect_to_server(slurmstepd_tbon_data.master_host, slurmstepd_tbon_data.port,
					_dstepd_client_handler, LAUNCHER, NULL,
					&(slurmstepd_tbon_data.master_host_server_index))){
			error("could not connect to stepd master host");
			return SLURM_ERROR;
		}
		...
		tbon_create->degree = 2; // going for minimum for testing
		xassert(tbon_create->degree >= 2); // no point in degree less than 2
		//tbon_create->total = job->step_ctx->step_resp->step_layout->node_cnt;
		//tbon_create->total = host_count; // TODO for het jobs, the host count will be different per step
		tbon_create->total = ctx->step_resp->step_layout->node_cnt;
		tbon_create->nodes = strdup(slurmstepd_tbon_data.nodes);

		if(dynpm_send_message_to_server(&tbon_header, (void**)&tbon_create,
					slurmstepd_tbon_data.master_host_server_index)){
			error("error sending stepd's tbon create master host %s", slurmstepd_tbon_data.master_host);
		} else {
			info("sent tbon create to stepd's master host %s", slurmstepd_tbon_data.master_host);
		}
	} else {
		error("stepd tbon was ready before being created");
	}

	_wait_tbon_cond_ready(&slurmstepd_tbon_data);
	info("done waiting for the dstepd tbon");
